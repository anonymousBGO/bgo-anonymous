...
from BGO import Attacker, ImageAttacker, TextAttacker
from dataset import paired_dataset, paired_dataset2
...
def retrieval_eval(model, ref_model, t_models, t_ref_models, t_test_transforms, data_loader, tokenizer, t_tokenizers,
    ...
    img_attacker = ImageAttacker(images_normalize, eps=args.eps / 255, steps=args.steps,
                                 step_size=args.step_size / 255, sample_numbers=args.sample_numbers,
                                 momentum_decay=args.momentum_decay,
                                 spatial_topk_ratio=args.spatial_topk_ratio,
    ...
    if args.scales is not None and args.scales.strip():
        scales = [float(itm) for itm in args.scales.split(',') if itm.strip()]
        print(scales)
    else:
        scales = None
    ...
    for batch_idx, (images, texts_group, images_ids, text_ids_groups, image_paths) in enumerate(data_loader):
        ...
        adv_images, adv_texts, execute_time = attacker.attack(images, texts, txt2img, all_texts, device=device,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_list', default='ALBEF,TCL,CLIP_ViT,CLIP_CNN', type=parse_model_list)
    ...
    parser.add_argument('--sample_numbers', default=5, type=int)
    parser.add_argument('--momentum_decay', default=1.0, type=float)
    parser.add_argument('--spatial_topk_ratio', default=0.8, type=float)
    parser.add_argument('--spatial_temperature', default=0.5, type=float)
    parser.add_argument('--spatial_beta', default=0.1, type=float)
    parser.add_argument('--result_file', default='./result_bgo.txt', type=str)
